...
//window.requestFileSystem(fileSystemType, 1024 * 1024, onGetFileSystemSuccess, onFileError);
	console.log("in filesysteminit");
  window.requestFileSystem(1, 0, gotFSSuccess, failed);
}
...
function fileReaderSuccess(file) {
    ...
    reader.onloadend = function(e) {
     ...
     console.log("read"+file_text);
	    
	     if (file_text.length==0) {
	  
	  file_text="[author]\nname=Pieter\n[user]";
	console.log("writing file_text"+file_text);
  }
...
function startINI() {
  fileSystemInit();
	console.log("after filesysteminit");
...
function INIset(isection,ikey,ivalue) {
    ...
    if (!sectionset) {
            ...
            theini.sections[theini.sections.length-1].items.push(new Property(ikey,ivalue));
            return true;
    }
    return false;
